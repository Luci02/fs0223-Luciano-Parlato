...
function whoIsBigger(n1, n2) {
  if (n1 > n2) {
    return `${n1} è maggiore`
  } else if (n1 == n2) {
    return `${n1} e ${n2} sono uguali`
  } else {
    return `${n2} è maggiore`
  }
}
...
function isThisAnEmail(str) {
  let emailValidation = new RegExp("/^[A-Za-z0-9_!#$%&'*+\/=?`{|}~^.-]+@[A-Za-z0-9.-]+$/", "gi");
...
function whatDayIsIt() {
  ...
  return today.toLocaleDateString("it-IT", option);
}
...
console.log(howManyDays('04/01/2023'));
...
//funziona con le date americane
console.log(isTodayMyBirthday('04/14/1950'));
...
*/
function addTestClassToTd() {
  let tr = document.querySelectorAll('tr');
  for (let currentTr of tr) {
